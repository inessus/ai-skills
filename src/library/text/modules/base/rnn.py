...
from library.utils.transforms.sequence import reorder_lstm_states, reorder_sequence
...
def lstm_encoder(sequence, lstm, seq_lens=None, init_states=None, embedding=None):
    ...
    """
        ...
        sequence 序列长度(batch_size, L_size, vec_size) 批次长度， 循环长度， 向量维度
    """
...
class StackedLSTMCells(nn.Module):
    """
        按照多层的存储连接起来 把LSTM快按宽度展开成多行， 并且尾部加入dropout层
    ...
    def forward(self, input_, state):
        """
        Arguments:
            input_: FloatTensor (batch, input_size)  (32, 128)
            states: tuple of the H, C LSTM states
                FloatTensor (num_layers, batch, hidden_size)  (1, 32, 256)
        Returns:
            LSTM states
            new_h: (num_layers, batch, hidden_size)  (1, 32, 256)
            new_c: (num_layers, batch, hidden_size)  (1, 32, 256)
        """
        hs = []
        cs = []
        for i, cell in enumerate(self._cells):  # 逐层求解参数，并把每层参数堆叠起来
