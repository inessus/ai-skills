...
class GreedySearchDecoder(nn.Module):
    ...
    def forward(self, input_seq, input_length, max_length):
        ...
        encoder_outputs, encoder_hidden = self.encoder(input_seq, input_length) # (T,1,N) (2*L,1,N)
        # Prepare encoder's final hidden layer to be first hidden input to the decoder
        decoder_hidden = encoder_hidden[:self.decoder.n_layers] # (L, 1, N)
        # Initialize decoder input with SOS_token
        decoder_input = torch.ones(1, 1, device=device, dtype=torch.long) * SOS_token # (1,1)
        ...
        for _ in range(max_length):
            ...
            decoder_output, decoder_hidden = self.decoder(decoder_input, decoder_hidden, encoder_outputs) # (1,7826),(L, 1, 500)
            # Obtain most likely word token and its softmax score
            decoder_scores, decoder_input = torch.max(decoder_output, dim=1) # 解码器输出的最大值和索引（即为字典中的索引）
            # Record token and score
            all_tokens = torch.cat((all_tokens, decoder_input), dim=0)
            all_scores = torch.cat((all_scores, decoder_scores), dim=0)
            # Prepare current token to be next decoder input (add a dimension)
            decoder_input = torch.unsqueeze(decoder_input, 0) # 转换成行向量
