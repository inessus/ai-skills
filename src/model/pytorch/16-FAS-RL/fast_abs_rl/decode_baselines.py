...
from decoding import Abstractor, DecodeDataset, Extractor, make_html_safe
...
def decode(save_path, abs_dir, ext_dir, split, batch_size, max_len, cuda):
    ...
    dec_log = {
        ...
        'extractor': (None if ext_dir is None
        ...
        for i_debug, raw_article_batch in enumerate(loader):
            ...
            for j, n in ext_inds:
                ...
                for k, dec_str in enumerate(decoded_sents):
                    with open(join(save_path, 'output_{}/{}.dec'.format(k, i)), 'w') as f:
...
if __name__ == '__main__':
    ...
    # decode options
    parser.add_argument('--batch', type=int, action='store', default=32, help='batch size of faster decoding')
    parser.add_argument('--n_ext', type=int, action='store', default=4, help='number of sents to be extracted')
    parser.add_argument('--max_dec_word', type=int, action='store', default=30, help='maximun words to be decoded for the abstractor')
    ...
    decode(args.path, args.abs_dir, args.ext_dir, data_split, args.batch, args.max_dec_word, args.cuda)
