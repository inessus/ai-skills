...
class RLDataset(JsonFileDataset):
    """
...
def configure_net(abs_dir, ext_dir, cuda):
    ...
    net_args = {
        'abstractor': (None if abs_dir is None
                               else json.load(open(join(abs_dir, 'meta.json')))),
        'extractor': json.load(open(join(ext_dir, 'meta.json')))
    }
...
def train(args):
    ...
    # 配置训练设置 configure training setting
    assert args.stop > 0
...
if __name__ == '__main__':
    ...
    # model options
    parser.add_argument('--abs_dir', action='store', help='pretrained summarizer model root path')
    parser.add_argument('--ext_dir', action='store', help='root of the extractor model')
    parser.add_argument('--ckpt', type=int, action='store', default=None, help='ckeckpoint used decode')

    # training options
    parser.add_argument('--reward', action='store', default='rouge-l', help='reward function for RL')
    parser.add_argument('--lr', type=float, action='store', default=1e-4, help='learning rate')
    ...
    parser.add_argument('--gamma', type=float, action='store', default=0.95, help='discount factor of RL')
    parser.add_argument('--stop', type=float, action='store', default=1.0, help='stop coefficient for rouge-1')
